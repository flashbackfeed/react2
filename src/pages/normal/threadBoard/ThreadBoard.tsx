...
function ThreadBoard() {
 ...
 const [message, setMessage] = useState<string>("");
  ...
  const getThreadBoard = (tid: string) => {
    ThreadBoardService.get(tid)           // 백엔드로 상세조회 요청
      .then((response: any) => {
  ...
  const updateThreadBoard = () => {
    ThreadBoardService.update(threadBoard.tid, threadBoard) // 백엔드로 수정요청
      .then((response: any) => {
        console.log(response.data);
...
const deleteThreadBoard = () => {
    ThreadBoardService.removeThread(threadBoard.tgroup) // 벡엔드로 삭제요청
      .then((response: any) => {
        console.log(response.data);
        // 페이지 이동
